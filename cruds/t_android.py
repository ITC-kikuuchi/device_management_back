...
def createAndroid(db: Session, android: android_schema.createAndroid, current_user):
    db_android = android_model.T_android(
        **android.dict(), create_id=current_user.id, update_id=current_user.id
    )
    db.add(db_android)
    db.commit()
...
def getDetailAndroid(db: Session, android_id: int):
    return (
        db.query(android_model.T_android)
        .filter(android_model.T_android.id == android_id)
        .first()
    )
...
def updateAndroid(
    db: Session,
    android: android_schema.updateAndroid,
    current_user,
    original: android_model.T_android,
):
...
# Android削除
def deleteAndroid(db: Session, original: android_model.T_android):
    db.delete(original)
    db.commit()
...
def getLastUpdatedData(db: Session):
    return (
        db.query(android_model.T_android)
        .filter(android_model.T_android.last_updated_flag == True)
        .first()
    )
