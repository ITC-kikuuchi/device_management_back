...
def createPc(db: Session, pc: pc_schema.createPc, current_user):
    db_pc = pc_model.T_pc(
        **pc.dict(), create_id=current_user.id, update_id=current_user.id
    )
    db.add(db_pc)
    db.commit()
    db.refresh(db_pc)
...
# PC更新
def updatePc(
    db: Session, pc: pc_schema.updatePc, current_user, original: pc_model.T_pc
):
...
def getLastUpdatedData(db: Session):
    return (
        db.query(pc_model.T_pc).filter(pc_model.T_pc.last_updated_flag == True).first()
    )
