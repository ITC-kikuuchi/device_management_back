...
def createIos(db: Session, ios: ios_schema.createIos, current_user):
    db_ios = ios_model.T_ios(
        **ios.dict(), create_id=current_user.id, update_id=current_user.id
    )
    db.add(db_ios)
    db.commit()
    db.refresh(db_ios)
...
# iOS更新
def updateIos(
    db: Session, ios: ios_schema.updateIos, current_user, original: ios_model.T_ios
):
...
def getLastUpdatedData(db: Session):
    return (
        db.query(ios_model.T_ios)
        .filter(ios_model.T_ios.last_updated_flag == True)
        .first()
    )
